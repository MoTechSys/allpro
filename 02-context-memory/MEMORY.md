...
> ⚠️ **تنبيه صلاحية (2026-09-01):** الأقسام البيئية أدناه («البيئة والمحركات»، Caddy، المسارات المحلية) كُتبت من بيئة وكيل سابقة (VM أخرى) — **لا تصلح لأي جلسة جديدة**. الصالح دائماً: قيود المالك، الاكتشافات المدللة، قواعد السلوك. ابدأ دائماً من `BOOTSTRAP.md` ثم `STATE.md`.
...
> ⚠️ **تحذير صلاحية (2026-09-02):** الأسطر التالية تاريخية — المسارات `~/.openclaw/...` ورابط zip وفرع `wave-0-architecture` لم تعد صالحة. المصدر الحي: `STATE.md` + `01-keif-aldiafa/reports/18`.
